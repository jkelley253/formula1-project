"""Prepare or deploy the existing drivers stack without putting passwords in CLI arguments.
...
SERVICE = ROOT / "backend/services/drivers"
...
def main() -> None:
    ...
    parser.add_argument("--stack", default="formula1-drivers-service")
    ...
    credentials = json.loads(capture([
        ...
        "sudo -n cat /etc/formula1-driver-sync/reader.json",
    ]))
    if credentials.get("username") != "drivers_api" or credentials.get("dbname") != "formula1" or not credentials.get("password"):
    ...
    with tempfile.TemporaryDirectory(prefix="formula1-api-package-") as temporary:
        ...
        capture([
            ...
            "--s3-bucket", bucket, "--s3-prefix", "formula1-drivers-service",
            "--region", args.region, "--output-template-file", str(template_path),
        ], env=sam_env, cwd=SERVICE)
        template = template_path.read_text()

    values = {
        "DriversVpcId": instance["VpcId"],
        # Same AZ as this single EC2 database avoids unnecessary cross-AZ traffic.
        "DriversApiSubnetIds": instance["SubnetId"],
        "DatabaseSecurityGroupId": groups[0]["GroupId"],
        "DriversDatabaseHost": instance["PrivateIpAddress"],
        "DriversDatabasePort": "5432", "DriversDatabaseName": credentials["dbname"],
        "DriversDatabaseUser": credentials["username"], "DriversDatabasePassword": credentials["password"],
        "EnableDriverSync": "false",
    }
    ...
    change = cloudformation.create_change_set(**{
        "StackName": args.stack, "ChangeSetName": f"database-reader-{time.time_ns()}",
        "ChangeSetType": "UPDATE", "TemplateBody": template,
    ...
    if args.execute:
        ...
        print("Updating the drivers stack...", flush=True)
        aws("cloudformation", "wait", "stack-update-complete", "--stack-name", args.stack)
        print("Drivers API deployment complete.", flush=True)
    else:
